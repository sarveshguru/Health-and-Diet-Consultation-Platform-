...
exports.getCurrentDietician = async (req, res) => {
  try {
    const dietician = await Dietician.findById(req.user.id).select("-password");
    if (!dietician) return res.status(404).json({ message: "Not found" });
    res.json(dietician);
  } catch (error) {
...
// controllers/dieticianController.js
// const getDieticianProfile = async (req, res) => {
//   try {
//     const dietician = await Dietician.findById(req.user.id).select("-password");
//     res.json(dietician);
//   } catch (error) {
