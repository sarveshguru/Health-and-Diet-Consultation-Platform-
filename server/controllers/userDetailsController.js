...
const submitUserDetails = async (req, res) => {
  try {
    const existing = await UserDetails.findOne({ userId: req.user.id });
    if (existing) {
      // If already exists, update by pushing new values to array fields
      const update = {};
      // For array fields, push new value if provided
      ["height", "weight", "sleepDuration", "waterIntake"].forEach((field) => {
        if (req.body[field] !== undefined) {
          // Always flatten the array to avoid nested arrays
          update[field] = [
            ...existing[field],
            ...[].concat(req.body[field])
          ].filter((v) => v !== undefined && v !== null && v !== "");
